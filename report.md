...
На кривом множестве глобальное $v$ быстро перестаёт быть касательным. Один прыжок $h+\alpha v$ выводит точку с поверхности, поэтому тот же сдвиг можно пройти мелкими шагами и после каждого пересчитать базис:
...
Геометрия ничего не учит: она двигает точку по уже собранному облаку. Поэтому дальше я учил возврат. Сеть получала $h+\varepsilon$ и восстанавливала $h$. Чтобы размер не смешивался с задачей, одну сеть брал узким MLP, другую шире и глубже. Это аддитивный денойзер.
...
Этот вариант я проверил на аддитивном денойзере. Свойство росло, правдоподобие падало сильнее обычного возврата, поэтому в сравнение ниже он не вошёл.
