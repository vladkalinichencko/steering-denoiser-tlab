...
from tmp.diagnostics import PROMPTS, continuation_nll, generate, sentiment
...
def evaluate(name, mode, ratio, state, capacity):
    ...
    texts = [generate(source, prompt, hook(mode["apply"], strength, target, i), i)
             for i, prompt in enumerate(PROMPTS)]
    scores = sentiment(texts)
    nll = sum(continuation_nll(source, prompt, text)
              for prompt, text in zip(PROMPTS, texts)) / len(texts)
