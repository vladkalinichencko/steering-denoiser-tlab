...
| 2 | Noise tolerance | \(x=h+\sigma\epsilon\); найти, когда random noise меняет NLL, generation и downstream state | [screen](tmp/screening.py) | предварительный Mac | 3 prompts, seed 0, полный \(r\)-grid | NLL 2.397→4.174→6.786 при \(r=0,0.8,2.0\); geometry curves записаны | — | [HTML](runs/mac_screening/screening.html) |
| 3 | Naive | \(R(x)=x,\ x=h+\alpha v\); общая точка отсчёта | [screen](tmp/screening.py) | предварительный Mac | 3 prompts, seed 0, полный \(r\)-grid | при \(r=0.8\): NLL 4.082, positive 0.973 | — | [HTML](runs/mac_screening/screening.html) |
| 4 | Additive MSE | Fixed-noise regression \(D(h+\sigma\epsilon)\to h\); отдельно simple MLP и capacity-matched GLP body, оба без timestep | [model](tmp/methods.py), [train](tmp/training.py), [run](tmp/run_mac_baselines.py) | предварительный Mac | FineWeb 100k/2k activations, 2000 updates, MPS; полный \(r\)-grid | simple val 1.039→0.636, capacity 1.333→0.487; при \(r=0\) NLL 3.513/3.357 против clean 2.397 | — | [HTML](runs/mac_screening/screening.html) |
| 5 | Interpolation MSE | \(z_t=(1-t)h+t\epsilon\); сеть получает \(z_t,t\) и предсказывает \(h\) за один вызов | [model](tmp/methods.py), [train](tmp/training.py) | предварительный Mac | FineWeb 100k/2k activations, 2000 updates, MPS; полный \(r\)-grid | val 1.018→0.605; при \(r=0\) NLL 3.144 против clean 2.397 | — | [HTML](runs/mac_screening/screening.html) |
| 6 | [GLP](https://arxiv.org/abs/2602.06964) + SDEdit | \(u_\theta(z_t,t)\approx\epsilon-h\); steered state частично зашумляется, затем 20 Euler-шагов к \(t=0\) | [model](tmp/methods.py), [train](tmp/training.py) | предварительный Mac | FineWeb 100k/2k activations, 2000 updates, MPS; полный \(r\)-grid | val 2.321→1.712; при \(r=0.8\) NLL 4.173, positive 0.998, repair 58.7 ms/state | — | [HTML](runs/mac_screening/screening.html) |
| 7 | One-Euler GLP | Один большой шаг \(z_0\approx z_t-t\,u_\theta(z_t,t)\); без нового обучения, контроль ошибки интегратора | [model](tmp/methods.py), [screen](tmp/screening.py) | предварительный Mac | GLP checkpoint; полный \(r\)-grid | при \(r=0.8\) NLL 4.191, positive 0.998, repair 3.0 ms/state против 58.7 ms у 20 шагов | — | [HTML](runs/mac_screening/screening.html) |
| 8 | [Consistency model](https://arxiv.org/abs/2303.01469) | Endpoint network \(f_\theta(z_t,t)\to z_0\); точки одной EMA trajectory дают одинаковый endpoint, \(f(z_0,0)=z_0\) задаётся skip connection | [model](tmp/methods.py), [run](tmp/run_mac_speedups.py) | предварительный Mac | FineWeb 20k/1k activations, 300 updates, CPU; полный \(r\)-grid | val 0.00716→0.00606; при \(r=0.8\) NLL 4.325, positive 0.995, repair 0.95 ms/state | — | [HTML](runs/mac_screening/screening.html) |
| 9 | [Rectified Flow](https://arxiv.org/abs/2209.03003) | 20-step GLP создаёт noise/endpoint pairs; новое velocity field выпрямляет эти paths; inference сравнивается при 1/2/4 шагах | [model](tmp/methods.py), [pairs/train](tmp/training.py), [run](tmp/run_mac_speedups.py) | предварительный Mac | FineWeb 20k/1k activations; 512/128 teacher pairs, seed 0; 300 updates, CPU; полный \(r\)-grid | val 0.287→0.130; 1/2/4-step curves и latency записаны, различия на 3 prompts нестабильны | — | [HTML](runs/mac_screening/screening.html) |
| 10 | [MeanFlow](https://arxiv.org/abs/2505.13447) | \(u_\theta(z_t,r,t)\) предсказывает average velocity на \([r,t]\), то есть displacement за один шаг; objective использует JVP \((v,0,1)\) и stop-gradient target | [model](tmp/methods.py), [run](tmp/run_mac_speedups.py) | предварительный Mac | FineWeb 20k/1k activations, 300 updates, CPU; полный \(r\)-grid | unweighted val MSE 1806.9→1693.4; при \(r=0.8\) NLL 4.325, positive 0.998, repair 0.93 ms/state | — | [HTML](runs/mac_screening/screening.html) |
| 11 | Nearest activation | Без обучения заменить \(x\) ближайшей real non-BOS activation | [geometry](tmp/methods.py), [screen](tmp/screening.py) | предварительный Mac | bank 20k, полный \(r\)-grid | уже при \(r=0\) NLL 5.637 против clean 2.397 | — | [HTML](runs/mac_screening/screening.html) |
| 12 | Segment-kNN | Среди real activations с проекцией на \([h,h+\alpha v]\) выбрать точку с минимальной perpendicular distance | [geometry](tmp/methods.py), [screen](tmp/screening.py) | предварительный Mac | bank 20k, полный \(r\)-grid | после \(r=0.2\) выбирает один endpoint: NLL 5.543, positive 0.651 на всех следующих \(r\) | — | [HTML](runs/mac_screening/screening.html) |
| 13 | Tangent/normal ablation | Для local SVD basis \(U(h)\): \(v_T=UU^Tv,\ v_N=(I-UU^T)v\); сравнить full, tangent и normal steering | [geometry](tmp/methods.py), [screen](tmp/screening.py) | предварительный Mac | bank 20k, \(k=256\), rank 16, полный \(r\)-grid | local tangent и normal curves записаны; tangent при \(r=0.8\) NLL 4.777, normal 4.129, naive 4.082 | — | [HTML](runs/mac_screening/screening.html) |
| 14 | Tangent-preserving MSE | \(\tau=UU^T\epsilon_1,\ \nu=(I-UU^T)\epsilon_2\); учить \(D(h+\tau+\nu)\to h+\tau\) | [train](train_denoiser.py) | не начато | — | — | — | — |
| 15 | Safe correction | Для \(c=D(x)-x\) использовать \(c_{safe}=c-\langle c,\hat v\rangle\hat v\), чтобы repair не стирал steering coordinate | [screen](tmp/screening.py) | предварительный Mac | capacity MSE, полный \(r\)-grid | при \(r=0.8\) NLL 5.371 против 4.537 у обычного capacity MSE при близком positive score | — | [HTML](runs/mac_screening/screening.html) |
| 16 | Local geodesic | \(h_{k+1}=h_k+\delta P_{T_{h_k}}v\); после шага пересчитать kNN и local SVD basis | [geometry](tmp/methods.py), [screen](tmp/screening.py) | предварительный Mac | bank 20k, 8 шагов, \(k=256\), rank 16, полный \(r\)-grid | при \(r=0.8\) NLL 4.218, positive 0.993, repair 93.5 ms/state против naive NLL 4.082 | — | [HTML](runs/mac_screening/screening.html) |
