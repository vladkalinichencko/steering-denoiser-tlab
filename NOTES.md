...
| 21 | Paired evaluator integrity | На текущей Git revision повторить identity control и сравнить методы на одинаковых random draws; при \(r=0\) identity methods должны дать побитово одинаковые continuations | [diagnostic](tmp/run_mac_geometry_diagnostic.py) | Mac выполняется | первые 16 фиксированных prompts, seed 0, \(r=0/0.4/0.8\) | — | — | [HTML](runs/mac_geometry_diagnostic/diagnostic.html) |
| 22 | Real-token geometry | На токенах тех же continuations сохранить вход и выход intervention; измерить steering projection, orthogonal displacement, kNN distance и local residual вместе с decoded outcome | [diagnostic](tmp/run_mac_geometry_diagnostic.py) | Mac выполняется | тот же paired diagnostic | — | — | [HTML](runs/mac_geometry_diagnostic/diagnostic.html) |
...
| 24 | Nonlinear calibration controls | Сопоставить запрошенную и достигнутую норму edit; отдельно проверить Curveball monotonicity, INNSteer identity/inverse и максимальный endpoint UniSteer | [diagnostic](tmp/run_mac_geometry_diagnostic.py) | Mac выполняется | 8 held-out activations и real response-token traces | — | — | [HTML](runs/mac_geometry_diagnostic/diagnostic.html) |
